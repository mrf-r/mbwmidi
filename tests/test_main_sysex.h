...
static void mainSysexTests()
{
    ...
    TEST_ASSERT(mts.cn == MIDI_CN_LOCAL);
    ...
    TEST_ASSERT(MIDI_RET_OK == midiSysexRead(&mts));
    TEST_ASSERT(mts.cin == MIDI_CIN_SYSEXEND3);
    TEST_ASSERT(mts.cn == MIDI_CN_UART1);
    TEST_ASSERT(mts.byte1 == 0x11);
    TEST_ASSERT(mts.byte2 == 0x19);
    TEST_ASSERT(mts.byte3 == 0xF7);
    ...
    TEST_TODO("sysex write while lock and release by other sysex source leads to corrupted (only last part of the sequence) ?");
    TEST_TODO("Flush");
    TEST_TODO("atomic?");
